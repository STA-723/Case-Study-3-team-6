...
ggplot(df[92:nrow(df),]) + geom_col(aes(names, missing)) + 
    labs(x = NULL, title = "Missing response per Question (Others)")
```
...
# Model: What do we want to fit?
    ...
    1. Sub-divide the participants into different groups. This depends on which questions they had to answer in Section C. The groups will be
        ...
        i. Had >=4 drinks in a row within last two weeks (habitual binge drinker)
        
        ii. Not above, but had drinks within 30 days (habitual drinker)
        
        iii. Drink in previous year but > 30 days ago (occasional drinker)
        
        iv. No drink in previous year (little to no drinker)
...
# Problems: Unreliable responses
Take A7, which allows multiple responses. There are at least 30 people who responded they live alone _and_ also live with someone else (roommate/spouse/parents, etc.). Part of this is a systematic misunderstanding of the survey instructions: there are at least 329 participants who thought even though they responded "None" to both C1 and C2, they still had to respond to C6. The unreliability of some of these responses must be accounted for. How?
...
#
group1 <- features %>% filter(C1!=1 | C2!=1)
group2 <- features %>% filter(C1==1, C2==1, C10 > 3)
group3 <- features %>% filter(C10 == 3)
group4 <- features %>% filter(C10 <= 2)
# Excluded: 72 people who do not meet the above filter (missing response in some of the questions)
#
# 2nd criterion (Question C7)
# Did someone make a mistake in responding to sections one is not expected to?
group2$unreliable <- ifelse(!is.na(group2$C6), 1, group2$unreliable) # 210
group3$unreliable <- ifelse(!is.na(group3$C6), 1, group3$unreliable) # 85
group4$unreliable <- ifelse(!is.na(group4$C6), 1, group4$unreliable) # 34
# For group4, exist people who reponded to group 1/2/3 section (C17 ~ C21)
group4$unreliable <- ifelse(apply(group4[,34:73], 1, function(x) !all(is.na(x))), 1, group4$unreliable) # 250
```
