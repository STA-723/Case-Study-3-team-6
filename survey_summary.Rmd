...
features <- features[,!grepl("G19", variable_names)]
...
predictor_names <- c(predictor_names[!grepl("G19", predictor_names)], "prev_survey", "unreliable")
variable_names <- colnames(features) # modified
...
# Excluded: 56 people who do not meet the above filter (missing response in some of the questions)
#
...
So in total, we have `r nrow(group1) + nrow(group2) + nrow(group3) + nrow(group4)` cases for the analysis, sub-divided into four population groups, for each of which we fit a separate CFA/SEM model. For each of the above outlined four groups we have `r paste(c(nrow(group1), nrow(group2), nrow(group3), nrow(group4)), collapse = ", ")` cases, respectively. 
